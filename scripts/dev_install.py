...
def install_dev() -> None:
    ...
    python_packages = ["jupyter-collaboration-ui-rename", "jupyter-docprovider-rename", "jupyter-server-ydoc-rename"]
    ...
    for py_package in python_packages:
        ...
        if py_package in ["jupyter-server-ydoc-rename"]:
            execute(f"jupyter server extension enable {real_package_name}")

        # List of jupyterlab extensions
        if py_package in ["jupyter-collaboration-ui-rename", "jupyter-docprovider-rename"]:
            execute(
